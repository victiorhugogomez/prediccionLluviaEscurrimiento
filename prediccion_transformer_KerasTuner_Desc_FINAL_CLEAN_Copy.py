...
class TransformerHyperModel(kt.HyperModel):
    ...
    def barra():
        for _ in tqdm(iter(int, 1), desc="Buscando hiperparámetros"):
            if done:
                break
            time.sleep(1)
    done = True
...
# ---------- Función principal ----------
def main():
    # Eliminar carpeta de logs corruptos si existe
    log_dir = "ktuner_logs/transformer_autocompresion"
    ...
    TransformerHyperModel(X.shape[1:], y_train, y_test, X_train, X_test),
    objective=kt.Objective("score", direction="max"),
    max_trials=10,  # <-- controla cuántos modelos diferentes probar
    directory="ktuner_logs",
    project_name="transformer_autocompresion",
    overwrite=True
)
    ...
    print("\nEvaluando modelo final sobre conjunto de prueba...")
    start = time.time()
